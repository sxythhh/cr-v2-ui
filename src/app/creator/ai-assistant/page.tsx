...
export default function CreatorAiAssistantPage() {
  ...
  const triggerSuggestion = (s: string) => {
    setMessage(s);
    setView("thinking");
    setThinkingStep(0);
    setThinkingDuration(0);
    ...
    <div className="flex min-h-screen flex-col font-inter tracking-[-0.02em]">
      ...
      <div className="flex flex-1 flex-col justify-between px-4 pb-5 sm:px-5">
        {view === "welcome" ? (
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {SUGGESTIONS.map((s) => (
                <button
                  ...
                  className="flex h-6 items-center rounded-full border border-foreground/[0.06] bg-white px-2.5 text-xs font-medium tracking-[-0.02em] text-page-text shadow-[0_1px_2px_rgba(0,0,0,0.03)] transition-colors hover:bg-foreground/[0.02] dark:border-white/[0.06] dark:bg-card-bg dark:shadow-[0_1px_2px_rgba(0,0,0,0.15)] dark:hover:bg-white/[0.04]"
                >
                  {s}
                </button>
              ))}
          ...
          /* ── Response state ── */
          <div ref={scrollRef} className="-mx-4 flex flex-1 flex-col gap-3 overflow-y-auto overflow-x-hidden pt-4 scrollbar-hide sm:-mx-5">
            {/* User message */}
            ...
            {/* Streamed response */}
            <div className="flex items-start gap-1.5 px-4 sm:px-5">
              <RobotIconSmall />
            ...
            {textDone && (
              <div className="flex flex-col gap-3">
                <div
                  ref={carouselRef}
                  className="flex w-full snap-x snap-mandatory gap-2 overflow-x-auto scrollbar-hide [scroll-padding-inline:16px] sm:[scroll-padding-inline:20px]"
                >
                  {AI_CAMPAIGNS.map((c, i) => (
                    <div key={i} className={cn(
                      "flex w-[304px] shrink-0 snap-start snap-always flex-col gap-2",
                      i === 0 && "ml-4 sm:ml-5",
                      i === AI_CAMPAIGNS.length - 1 && "mr-4 sm:mr-5",
                    )}>
                      ...
                      <div className="overflow-hidden rounded-2xl border border-foreground/[0.06] bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)] dark:border-white/[0.06] dark:bg-card-bg dark:shadow-[0_1px_2px_rgba(0,0,0,0.15)]">
                        ...
                        <div className="flex flex-col gap-3 px-4 pb-4 pt-3">
                          <div className="flex items-center gap-1.5">
                            ...
                            {c.verified && <svg width="12" height="12" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="7" fill="url(#vbm2)"/><path d="M4.5 7.2L6 8.7L9.5 5.3" stroke="#252525" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/><defs><linearGradient id="vbm2" x1="7" y1="0" x2="7" y2="14"><stop stopColor="#FDDC87"/><stop offset="1" stopColor="#FCB02B"/></linearGradient></defs></svg>}
