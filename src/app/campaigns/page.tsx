...
import { TopUpModal } from "@/components/campaign-details/TopUpModal";
    ...
    <div className="group relative flex flex-col md:h-[185px] md:flex-row cursor-pointer items-stretch md:items-center gap-0 md:gap-4 rounded-[20px] border border-[rgba(37,37,37,0.06)] bg-[linear-gradient(86.46deg,rgba(255,255,255,0)_87.34%,rgba(0,178,110,0.07)_100%),#FFFFFF] shadow-[0_1px_2px_rgba(0,0,0,0.03)] dark:border-foreground/[0.03] dark:bg-foreground/[0.03] before:pointer-events-none before:absolute before:inset-0 before:rounded-[20px] before:bg-foreground/0 before:transition-colors before:duration-200 hover:before:bg-foreground/[0.03]">
      ...
      <div className="flex min-w-0 flex-1 flex-col md:flex-row items-stretch md:items-center gap-0 md:gap-4 md:self-stretch md:pr-4">
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-between gap-3 self-stretch px-4 py-3 md:px-0 md:py-4">
          ...
          {/* Title */}
          <h3 className="truncate font-inter text-base leading-normal font-semibold tracking-[-0.02em] text-page-text md:text-sm lg:text-base">
            {campaign.title}
          </h3>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <div className="flex items-center gap-1">
              ...
              <div className="flex h-6 items-center gap-1.5 rounded-full bg-foreground/[0.06] pl-2 pr-2.5 backdrop-blur-[12px] dark:border dark:border-foreground/[0.03] dark:bg-foreground/[0.03]">
                ...
                <span className="whitespace-nowrap font-inter text-xs font-medium tracking-[-0.02em] text-page-text">
                  {campaign.category}
                </span>
              </div>
            </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col md:flex-row items-stretch md:items-center gap-0 md:gap-4 md:self-stretch md:pr-4">
        ...
        <div className="flex min-w-0 flex-1 flex-col self-stretch px-4 py-3 md:px-0 md:py-3">
          ...
          <div className="flex flex-col gap-2">
            ...
            {/* Title */}
            <h3 className="truncate font-inter text-sm font-medium leading-normal tracking-[-0.02em] text-page-text">
              {campaign.title}
            </h3>
          </div>
          ...
          <div className="mt-auto flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              {[
                ...
                { value: campaign.videosSubmitted, label: "Videos submitted" },
                { value: campaign.views, label: "Views" },
              ].map((stat) => (
                <div
                  key={stat.label}
                  className={cn(
                    "flex min-w-0 flex-1 flex-col justify-center gap-1 overflow-hidden rounded-xl border border-[rgba(37,37,37,0.06)] px-2.5 py-2 dark:border-foreground/[0.03] dark:bg-foreground/[0.03]",
                    stat.highlight ? "bg-[rgba(255,144,37,0.1)]" : "bg-white",
                  )}
                >
                  <span className={cn("truncate font-inter text-sm font-medium tracking-[-0.02em]", stat.highlight ? "text-[#FF9025]" : "text-page-text")}>
                    {stat.value}
                  </span>
                  <span className="truncate font-inter text-[11px] tracking-[-0.02em] text-page-text/50">
                    {stat.label}
                  </span>
                </div>
              ))}
  ...
  return (
    <Link href={`/campaigns/${campaign.id}`} className="block">
      {campaign.status === "active" ? (
        <ActiveCampaignCard campaign={campaign} onTopUp={onTopUp} />
      ) : (
        <DetailCampaignCard campaign={campaign} />
      )}
    </Link>
  );
}
