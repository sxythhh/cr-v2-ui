...
function DurationIndicator({ fraction }: { fraction: number }) {
  ...
  return (
    <svg width="12" height="12" viewBox="0 0 12 12" className="shrink-0 -rotate-90">
      <circle cx="6" cy="6" r={r} fill="none" className="stroke-foreground/20" strokeWidth="1.33" />
      <circle cx="6" cy="6" r={r} fill="none" className="stroke-foreground" strokeWidth="1.33" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" />
    </svg>
  );
}
...
function ActionButton({ label, variant }: { label: string; variant: "default" | "green" | "primary" }) {
  const styles = {
    ...
    primary: "bg-foreground text-white dark:bg-white dark:text-[#111111]",
  }[variant];
      ...
      <div className={cn("flex flex-1 items-center transition-[border-color] duration-75", !isLast && cn("border-b", hideBorder ? "border-transparent" : "border-foreground/[0.03]"))}>
        ...
        <div className={cn("flex w-[244px] shrink-0 items-center gap-2 px-3 py-3", rowOpacity)}>
          ...
          <div className="flex min-w-0 flex-col gap-1.5">
            ...
            <span className="truncate font-inter text-xs leading-normal tracking-[-0.02em] text-page-text-muted">
              {contract.handle}
            </span>
          </div>
        </div>
        ...
        <div className={cn("flex min-w-0 flex-1 items-center justify-end px-3 py-3 pl-5", rowOpacity)}>
          <span className="truncate font-inter text-xs leading-normal tracking-[-0.02em] text-page-text text-right">
            {contract.type}
          </span>
        </div>
      ...
      <div className="relative flex max-h-[90vh] flex-col">
        ...
        <div className="flex shrink-0 items-center justify-between gap-4 bg-card-bg px-5 pb-5 pt-4">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <button
              type="button"
              className="flex h-10 cursor-pointer items-center gap-2 rounded-full bg-foreground px-4 font-inter text-sm font-medium tracking-[-0.02em] text-white transition-colors hover:bg-foreground/90 dark:bg-white dark:text-[#111111] dark:hover:bg-white/90"
            >
              <SendIcon />
              Send Contract
            </button>
